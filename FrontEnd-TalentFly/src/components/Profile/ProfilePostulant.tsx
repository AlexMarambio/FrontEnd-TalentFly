...
function ProfilePostulant() {
  ...
  useEffect(() => {
    const fetchDbUserInfo = async () => {
      try {
        const response = await axios.get('https://backend.talenfly.com/dashboard/postulante');
    ...
    <div className="App">
      ...
      <div className="text-center m-4">
        ...
        {isEditing ? (
          <form className='block rounded-3xl userCardTF mx-auto lg:flex'>
            <div className='relative bg-[#CEE9F5] shadow-lg rounded-3xl overflow-hidden border-2'>
              ...
              <div className='relative z-10 p-8'>
                <img src={editableUser.avatar} alt="Avatar" className="w-32 h-32 mx-auto rounded-full mb-4" />
                <input type="text" name="nick" value={editableUser.nick} onChange={handleEditChange} className="block mx-auto mb-4 p-2 border rounded" />
                <input type="email" name="email" value={editableUser.email} onChange={handleEditChange} className="block mx-auto mb-4 p-2 border rounded" />
                <textarea name="description" value={editableUser.description} onChange={handleEditChange} className="block mx-auto mb-4 p-2 border rounded" rows={4} />
                <input type="text" name="experience" value={editableUser.experience} onChange={handleEditChange} className="block mx-auto mb-4 p-2 border rounded" />
                <input type="text" name="studies" value={editableUser.studies} onChange={handleEditChange} className="block mx-auto mb-4 p-2 border rounded" />
                <button type="button" onClick={handleSaveChanges} className='bg-[#7a69de] text-white py-2 px-4 rounded-full mt-4 hover:bg-[#645FCB] transition ease-in-out delay-100 hover:scale-110 duration-150'>
                  Guardar Cambios
                </button>
              </div>
            </div>
          ...
          <form className='block rounded-3xl userCardTF mx-auto '>
          <div className='block lg:flex'>  
            <div className='relative bg-[#CEE9F5] shadow-lg rounded-3xl overflow-hidden border-2'>
              ...
              <div className='relative z-10 p-8'>
                ...
                <button onClick={() => setIsEditing(true)} className='bg-[#7a69de] text-white py-2 px-4 rounded-full mt-4 hover:bg-[#645FCB] transition ease-in-out delay-100 hover:scale-110 duration-150'>
                  ✏ Editar
                </button>
              </div>
            </div>
            ...
            <div className='block bg-[#cee9f5] shadow-lg rounded-lg p-8 userCardTF mx-auto mt-8 mb-8'>
              <p className='text-left'><strong>Perfil profesional:</strong> {user.studies}</p>
            </div>
            <div className='bg-[#cee9f5] shadow-lg rounded-lg p-8 userCardTF mx-auto'>
              <p className='text-left'><strong>Experiencia laboral:</strong> {user.experience}</p>
